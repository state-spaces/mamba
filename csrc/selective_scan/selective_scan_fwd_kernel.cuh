...
template<int kNThreads_, int kNItems_, int kNRows_, bool kIsEvenLen_,
         ...
         bool kHasZ_, bool kIsVarLen_, typename input_t_, typename weight_t_>
struct Selective_Scan_fwd_kernel_traits {
    ...
    static constexpr bool kDirectIOPos = kIsEvenLen && kNLoadsPos == 1;
    ...
    using BlockLoadPosIdsVecT = cub::BlockLoad<pos_vec_t, kNThreads, kNLoadsPos,
        !kDirectIOPos ? cub::BLOCK_LOAD_WARP_TRANSPOSE : cub::BLOCK_LOAD_DIRECT>;
...
void selective_scan_fwd_kernel(SSMParamsBase params) {
    ...
    constexpr bool kDirectIOPos = Ktraits::kDirectIOPos;
    ...
    for (int chunk = 0; chunk < params.n_chunks; ++chunk) {
        ...
        pos_t pos_ids_vals[kNRows][kNItems];
        __syncthreads();
        #pragma unroll
        for (int r = 0; r < kNRows; ++r) {
        ...
        u += kChunkSize;
        delta += kChunkSize;
        if constexpr (kIsVarLen) { pos_ids += kChunkSize; }
            ...
            BOOL_SWITCH(params.is_variable_C, kIsVariableC, [&] {
                BOOL_SWITCH(params.z_ptr != nullptr , kHasZ, [&] {
                    BOOL_SWITCH(params.pos_ids_ptr != nullptr , kIsVarLen, [&] {
                        using Ktraits = Selective_Scan_fwd_kernel_traits<kNThreads, kNItems, kNRows, kIsEvenLen, kIsVariableB, kIsVariableC, kHasZ, kIsVarLen, input_t, weight_t>;
