#!/usr/bin/env python3
"""Visual comparison of text generation latency across Mamba model sizes.

Loads multiple pretrained Mamba models, generates tokens from a prompt,
and produces a comparison chart of throughput, latency, and VRAM usage.

For single-model benchmarking with full CLI options, use
benchmark_generation_mamba_simple.py instead.

Usage:
    python benchmarks/benchmark_text_generation_latency_visual.py [--genlen N] [--dtype DTYPE]
"""
...
def plot_results(results, genlen):
...
def main():
    parser = argparse.ArgumentParser(description="Visual benchmark of text generation latency")
    parser.add_argument("--genlen", type=int, default=100, help="Number of tokens to generate")
    ...
    plot_results(results, args.genlen)
