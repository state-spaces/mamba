"""
GPU-compatible TileLang implementation of Mamba3 forward kernel with MIMO support.

This is a modified version of mamba3_mimo_fwd.py that removes Hopper-specific features
(TMA - Tensor Memory Accelerator) to enable compatibility with older GPU architectures
including Ampere, Ada, and Volta.

Key differences from original:
- Removed TL_DISABLE_TMA_LOWER pass config (TMA is Hopper-only)
- Simplified memory access patterns to avoid TMA dependencies
- Uses standard shared memory and registers instead of specialized hardware features
- Maintains mathematical correctness while sacrificing some performance optimizations
...
@tilelang.jit(
    out_idx=[],
    pass_configs={
        # Removed: tilelang.PassConfigKey.TL_DISABLE_TMA_LOWER (Hopper-only)
        tilelang.PassConfigKey.TL_DISABLE_WARP_SPECIALIZED: True,
        tilelang.PassConfigKey.TL_ENABLE_FAST_MATH: True,
    })
def mamba_mimo_fwd_compat(
    B,
    S,
    H,
    G,
    ...
    threads: int = 128,
    num_stages: int = 1,  # Reduced from 0 for better compatibility
) -> torch.Tensor:
    """
    GPU-compatible forward kernel for Mamba3 MIMO.
    ...
    Args:
        ...
        P: Head dimension
        R: MIMO rank
        hasZ: Whether Z tensor is provided
        hasD: Whether D tensor is provided
        reduceO: Whether to apply output projection
        chunk_size: Chunk size for processing
        rotary_dim_divisor: Divisor for rotary dimension
        dtype: Data type ('bfloat16', 'float16', 'float32')
        num_stages: Pipeline stages (reduced for compatibility)
    """
    ...
    @T.prim_func
    def mamba_mimo_fwd_kernel_compat(
            Q: T.Tensor([B, S, R, G, N], dtype),
            K: T.Tensor([B, S, R, G, N], dtype),
            V: T.Tensor([B, S, H, P], dtype),
            O: T.Tensor(O_shape, dtype),
            Q_BIAS: T.Tensor([H, R, N], T.float32),
            K_BIAS: T.Tensor([H, R, N], T.float32),
            MIMO_V: T.Tensor([H, R, P], T.float32),
            MIMO_O: T.Tensor([H, R, P], T.float32),
            OUT_NORM_WEIGHT: T.Tensor([H, P], T.float32),
            Z: T.Tensor([B, S, H, P], dtype),
            D: T.Tensor([H], T.float32),
            MIMO_Z: T.Tensor([H, R, P], T.float32),
            ANGLES: T.Tensor([B, S, H, N//rotary_dim_divisor], T.float32),
            DA_CS: T.Tensor([B, H, S], T.float32),
            DA_CS_REV: T.Tensor([B, H, S], T.float32),
            DT: T.Tensor([B, H, S], T.float32),
            TRAP: T.Tensor([B, H, S], dtype),
            SEGSUM: T.Tensor([B, H, nchunks, chunk_size, chunk_size], T.float32),
            FINAL_STATE: T.Tensor([B, H, N, P], T.float32),
            FINAL_K: T.Tensor([B, R, H, N], dtype)
            ):
        """
        Fused chunked forward pass for Mamba3 MIMO with broader GPU compatibility.
        
        This kernel avoids hardware-specific features and uses standard memory access
        patterns suitable for all modern NVIDIA GPUs (Ampere onwards).
        """
        ...
        with T.Kernel(H, B, threads=threads) as (i_h, i_b):
            ...
            # --- Buffer Allocation (standard shared memory, no TMA) ---
            ...
            # --- Swizzling Annotation (standard, no TMA-specific patterns) ---
            T.annotate_layout({
            ...
            # --- Chunk Loop (compatible with all GPUs) ---
            for i in T.Pipelined(0, nchunks, num_stages=num_stages):
                ...
                # --- Cache full QK dot for inter-chunk contribution ---
                ...
                # Strictly causal mask over chunk steps
    ...
    return mamba_mimo_fwd_kernel_compat


def mamba_mimo_forward_compat(
    q, k, v, 
    q_bias, k_bias, 
    mimo_v, mimo_o, 
    z, D, 
    ...
    num_stages=1):
    """
    Wrapper for GPU-compatible MIMO forward pass.
    
    This version uses the compatibility kernel that avoids Hopper-specific features.
    """
    B, S, R, G, N = q.shape
