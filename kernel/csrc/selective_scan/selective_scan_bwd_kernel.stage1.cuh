...
void selective_scan_bwd_kernel(SSMParamsBwd params) {
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int i = 0; i < kNItems; ++i) {
            ...
            delta_vals[i] = float(delta_vals_load[i]) + (delta_bias == nullptr ? 0 : delta_bias[r]);
        ...
        #pragma unroll
        for (int i = 0; i < kNItems; ++i) { du_vals[i] = (D_val == nullptr ? 0 : D_val[r]) * dout_vals[i]; }
        #pragma unroll
        for (int i = 0; i < kNItems; ++i) { dD_val[r] += dout_vals[i] * float(u_vals[i]); }
        ...
        if constexpr (kDeltaSoftplus) {
            ...
            #pragma unroll
            for (int i = 0; i < kNItems; ++i) {
                float delta_val = float(delta_vals_load[i]) + (delta_bias == nullptr ? 0 : delta_bias[r]);
        ...
        for (int i = 0; i < kNItems; ++i) { ddelta_bias_val[r] += ddelta_vals[i]; }
...
void selective_scan_bwd_cuda(SSMParamsBwd &params, cudaStream_t stream) {
    if (params.seqlen <= 128) {
        selective_scan_bwd_launch<32, 4, 1, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 256) {
        selective_scan_bwd_launch<32, 8, 1, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 512) {
        selective_scan_bwd_launch<32, 16, 1, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 1024) {
        selective_scan_bwd_launch<64, 16, 1, input_t, weight_t>(params, stream);
    } else {
        selective_scan_bwd_launch<128, 16, 1, input_t, weight_t>(params, stream);
    }
}